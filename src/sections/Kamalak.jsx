...
const Kamalak = ({ setActiveSection }) => {
  const { t } = useLanguage();
  const [kamalak, setkamalak] = useState(null);
    ...
    <div className="bg-white">
      ...
      <div className="max-w-[1920px] mx-auto px-5 sm:px-5 md:px-7 lg:px-[50px] 2xl:px-[100px] pt-[120px] sm:pt-[140px]">
        ...
        >
          ...
          <span className="text-base sm:text-lg font-medium">{t("kamalak.back")}</span>
        </button>
      </div>
      ...
      "
        ...
        "
            >
              {kamalak && kamalak.videos && kamalak.videos.length > 0 ? (
                <img
                  src={kamalak.videos[currentVideoIndex]}
            ...
            {kamalak && kamalak.videos && kamalak.videos.length > 1 && (
              <div className="flex justify-center gap-2 mt-4">
                {kamalak.videos.map((_, index) => (
                  <button
                    key={index}
